...
export function createMaterialsRouter({
  ...
  function storeOf(material) {
    ...
    if (!store) throw httpError('Google Drive nije povezan. Otvori tab Literatura i klikni "Poveži Google Drive".', 503);
    return store;
  }
  ...
  function decorate(req, user, material) {
    ...
    // Google Drive link: SAMO administrator — učenik ga nikad ne dobija
    // (čita i preuzima kroz našu stranicu, ne treba mu pristup Drive-u)
    const driveLinks = store?.linkFor ? store.linkFor(material) : null;
    if (driveLinks && isAdmin) out.drive = driveLinks;
  ...
  async function serveFile(req, res, { id, material, download = false, inline = false }) {
    ...
    if (!buffer || buffer.length === 0)
      throw httpError('Fajl nije pronađen na Google Drive-u. Obriši materijal i dodaj ga ponovo.', 410);
  ...
  router.post('/api/materials', upload.single('file'), async (req, res) => {
    ...
    try {
      ...
      const razrediArr = parseRazredi(razredi);

      materialId = newId();
      store      = (primaryStore && primaryStore()) || storeForMaterial({});   // Drive, ako je povezan
      if (!store)
        throw httpError('Google Drive nije povezan. Otvori tab Literatura i klikni "Poveži Google Drive".', 503);
  ...
  router.put('/api/materials/:id', async (req, res) => {
    try {
      ...
      if (razredi !== undefined)      upd.razredi      = parseRazredi(razredi);
