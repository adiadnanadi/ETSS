...
export function createDriveStore(driveOrGetter, cfg = {}) {
  const {
    label = 'Google Drive',
    shareAnyoneDefault = process.env.GDRIVE_SHARE_ANYONE !== '0'
  } = cfg;

  // Klijent može biti fiksiran ili se tražiti u hodu — kad se Drive poveže iz
  // panela, veza odmah proradi bez restarta servera.
  const client = () => {
    const d = typeof driveOrGetter === 'function' ? driveOrGetter() : driveOrGetter;
    if (!d) throw Object.assign(
      new Error('Google Drive nije povezan. Otvori tab Literatura i klikni "Poveži Google Drive".'),
      { status: 503 }
    );
    return d;
  };
  ...
  const store = {
    kind:  'drive',
    label,
    get mode() { return client().mode; },
    get ready() { try { return !!client(); } catch { return false; } },
    ...
    async removeMaterial(material = {}) {
      if (material.driveFileId) await client().remove(material.driveFileId);
    },

    // ── Čitanje (za skidanje fajla kroz našu stranicu) ───────────────────────
    read: (material) => client().download(typeof material === 'string' ? material : material.driveFileId),

    // Fajl ide direktno iz Google-a ka browseru (Range prolazi kroz, PDF radi).
    streamTo: (req, res, material, opts = {}) => client().proxyFile(req, res, {
    ...
    syncMeta(material = {}, upd = {}) {
      ...
      return client().updateMeta(material.driveFileId, { name, description }).catch(e => {
